    ...
    public class OverlayForm : Form
    {
        ...
        public OverlayForm(Rectangle bounds, Rectangle workingArea)
        {
            ...
            _borderRect = new Rectangle(
                workingArea.X - bounds.X + ExtraSideClearance,
                workingArea.Y - bounds.Y + ExtraTopClearance,
                workingArea.Width - ExtraSideClearance * 2,
                workingArea.Height - ExtraTopClearance);
        ...
        private void DrawPerimeterText(Graphics g, Rectangle rect, int t)
        {
            ...
            {
                double s = i * spacing;
                var (x, y, angle) = PointOnPerimeter(s, rect, t);
        ...
        // Walks clockwise around the border centerline starting at top-left of the rect.
        // Returns the point and the rotation angle so text stays upright on each edge.
        private (float x, float y, float angle) PointOnPerimeter(double s, Rectangle rect, int t)
        {
            float half = t / 2f;
            double top = rect.Width;
            double right = rect.Height;
            double bottom = rect.Width;

            if (s < top)
                return (rect.X + (float)s, rect.Y + half, 0f);
            s -= top;
            if (s < right)
                return (rect.X + rect.Width - half, rect.Y + (float)s, 90f);
            s -= right;
            if (s < bottom)
                return (rect.X + rect.Width - (float)s, rect.Y + rect.Height - half, 180f);
            s -= bottom;
            return (rect.X + half, rect.Y + rect.Height - (float)s, 270f);
        }
    }
}
